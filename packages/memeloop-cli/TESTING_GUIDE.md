...
- Node.js installed
- memeloop-cli built (`pnpm -C packages/memeloop-cli build`)
- puppeteer installed (`pnpm -C packages/memeloop-cli install`)
...
- `[memeloop-cli] Starting node: worker-node`
- `[memeloop-cli] WebSocket server listening on port 38473`
- `[memeloop-cli] mDNS service published`
- `[memeloop-cli] LAN PIN mode enabled (pin: 123456)`
...
- `[memeloop-cli] Starting node: control-node`
- `[memeloop-cli] WebSocket server listening on port 38472`
- `[memeloop-cli] mDNS service published`
- `[memeloop-cli] Discovered peer: worker-node (LAN)`
- `[memeloop-cli] LAN PIN pairing successful with worker-node`
...
```
[memeloop-cli] Peer online: worker-node (nodeId: <id>)
```

Check Node B logs for:

```
[memeloop-cli] Peer online: control-node (nodeId: <id>)
```
